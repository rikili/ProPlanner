    ...
    const updateSelections = (cursorDateHalf) => {
        ...
        while (cursorDateHalf >= iterHalfDate) {
            if (isDateValid(iterHalfDate)) {
                ...
                if (onFirstHalf) {
                    newSelections[getMonthIndex(iterHalfDate)][iterHalfDate.getDate() - 1][0] =
                        isAddingSelect;
                    iterHalfDate = getHalfDate(iterHalfDate, false);
                    onFirstHalf = false;
                } else {
                    newSelections[getMonthIndex(iterHalfDate)][iterHalfDate.getDate() - 1][1] =
                        isAddingSelect;
                    onFirstHalf = true;
        ...
        if (decisionEditRange) {
            dispatch(
                setPlanDecision({
                    ...
                    range: [decisionEditRange[0].toISOString(), decisionEditRange[1].toISOString()],
                })
            );
        }
        setDecisionStart(null);
