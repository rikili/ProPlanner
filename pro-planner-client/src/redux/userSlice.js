...
export const getUserAsync = createAsyncThunk(
    ...
    async ({ planId }, { rejectWithValue }) => {
        return axios
            .get(buildServerRoute('user'), { params: { eventId: planId } })
            .then((result) => result.data)
            .catch((e) => rejectWithValue(e.response.status));
    }
);

export const addUserAsync = createAsyncThunk('user/add', async ({ planId, newUser }) => {
...
const userSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(deleteUsersAsync.fulfilled, (state, action) => {
            const updatedUsers = state.userList.filter((user) => !action.payload.includes(user));
