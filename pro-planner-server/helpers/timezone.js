...
const makeAvailabilityDates = (date, timezone) => {
    ...
    if (offset <= -6) {
        ...
        for (let i = 0; i < months.length; i++) {
            ...
            const daysOfMonth = date_fns.getDaysInMonth(
                new Date(`${parseInt(monthDateSplit[0]) + 1}-2-${monthDateSplit[1]}`)
            );
        ...
        if (isFirstMonthDaySelected) {
            date[`${prevMonth}-${year}`][date_fns.getDaysInMonth(prevMonthDate) - 1] = [
                false,
                true,
            ];
        }
        ...
        for (let i = 0; i < months.length; i++) {
            const monthDateSplit = months[i].split('-');
            daysOfMonth = date_fns.getDaysInMonth(
                new Date(`${parseInt(monthDateSplit[0]) + 1}-2-${monthDateSplit[1]}`)
            );
...
const createDateShift = (date, shiftType) => {
    ...
    if (shiftType === 'next') {
        ...
        return [
            parseInt(month) === 0 ? '11' : month - 1,
            parseInt(month) === 0 ? parseInt(year) - 1 : year,
        ];
    }
};
